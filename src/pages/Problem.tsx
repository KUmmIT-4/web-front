        ...
        answerOptions: [
          {
            ...
            isCorrect: true,
            rationale:
              "BFS는 큐(Queue) 자료구조를 사용하여 너비 우선으로 탐색하며, 큐에 요소를 추가할 때는 push 메소드를 사용합니다.",
          },
          {
            text: "stack / push",
            isCorrect: false,
            rationale:
              "스택(Stack)은 깊이 우선 탐색(DFS)에 주로 사용되며, BFS에는 적합하지 않습니다.",
          },
          {
            text: "vector / push_back",
            isCorrect: false,
            rationale:
              "벡터(Vector)는 동적 배열이며, 큐처럼 선입선출 구조를 직접 제공하지 않습니다.",
          },
          {
            text: "list / emplace_back",
            isCorrect: false,
            rationale:
              "리스트(List)도 선입선출 구조를 직접 제공하지 않으며, BFS에는 큐가 적합합니다.",
          },
        ],
      },
      {
        ...
        answerOptions: [
          {
            ...
            isCorrect: true,
            rationale:
              "BFS의 while 루프는 큐가 빌 때까지 반복하며, q.front()로 요소를 확인한 후 q.pop()으로 제거합니다.",
          },
          {
            text: "q.size() > 0 / erase",
            isCorrect: false,
            rationale:
              "q.size() > 0은 유효하지만, 큐에서는 erase가 아닌 pop을 사용합니다.",
          },
          {
            text: "q / remove",
            isCorrect: false,
            rationale:
              "큐 자체를 조건으로 사용할 수 없으며, remove는 큐의 표준 메소드가 아닙니다.",
          },
          {
            text: "q.count() > 0 / delete",
            isCorrect: false,
            rationale:
              "count()는 큐의 메소드가 아니며, delete는 메모리 해제에 사용됩니다.",
          },
        ],
      },
      {
...
\`\`\``,
        ...
        answerOptions: [
          {
            text: "next_x >= 0 && next_x < M && next_y >= 0 && next_y < N && field[next_x][next_y] == 1 && !visited[next_x][next_y]",
            isCorrect: true,
            rationale:
              "범위 체크, 배추 존재 확인, 방문 여부 확인을 모두 포함한 완전한 조건입니다.",
          },
          {
            text: "next_x >= 0 && next_x < M && next_y >= 0 && next_y < N",
            isCorrect: false,
            rationale:
              "범위만 확인하고 배추 존재나 방문 여부를 확인하지 않아 부정확합니다.",
          },
          {
            text: "field[next_x][next_y] == 1 && !visited[next_x][next_y]",
            isCorrect: false,
            rationale: "범위 체크가 없어 배열 범위를 벗어날 수 있습니다.",
          },
          {
            text: "visited[next_x][next_y] == false",
            isCorrect: false,
            rationale:
              "방문 여부만 확인하고 범위나 배추 존재를 확인하지 않습니다.",
          },
        ],
      },
      {
...
\`\`\``,
        ...
        answerOptions: [
          {
            text: "field[i][j] == 1 && !visited[i][j]",
            isCorrect: true,
            rationale:
              "배추가 있고 아직 방문하지 않은 곳에서만 BFS를 시작하여 새로운 배추 군락을 찾습니다.",
          },
          {
            text: "field[i][j] == 1",
            isCorrect: false,
            rationale:
              "이미 방문한 배추에 대해서도 중복으로 BFS를 호출할 수 있습니다.",
          },
          {
            text: "!visited[i][j]",
            isCorrect: false,
            rationale: "배추가 없는 빈 칸에서도 BFS를 호출할 수 있습니다.",
          },
          {
            text: "visited[i][j] == false",
            isCorrect: false,
            rationale:
              "!visited[i][j]와 같은 의미이며, 배추 존재 확인이 빠져있습니다.",
          },
        ],
      },
    ],
  ...
  return (
    <QuizLayout
      ...
      problemDescriptionRef={
        problemDescriptionRef as React.RefObject<HTMLDivElement>
      }
